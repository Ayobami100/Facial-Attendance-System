...
{
    public partial class Training_Form : MetroFramework.Forms.MetroForm
    {
        ...
        public Training_Form(Form2 _father)
        {
            InitializeComponent();
            father = _father;
            Face = father.Face;
        ...
        private bool save_training_data(Image face_data)
        {
            if (txtLas.Text != "" && txtFir.Text != "")
                try
                {
                    ...
                    cmd.Parameters.Add("@LastName", SqlDbType.NVarChar, 50).Value = txtLas.Text;
                    cmd.Parameters.Add("@FirstName", SqlDbType.NVarChar, 50).Value = txtFir.Text;
                    cmd.Parameters.Add("@OtherName", SqlDbType.NVarChar, 50).Value = txtOth.Text;
            ...
            try
            {
                string Newfacename = txtFir.Text +" "+ txtLas.Text + " " + txtOth.Text;
                Random rand = new Random();
                bool file_create = true;
                string facename = "face_"+" " +Newfacename+  "_" + rand.Next().ToString() + ".jpg";
                while (file_create)
                {
                    ...
                    {
                        facename = "face_" + Newfacename + "_" + rand.Next().ToString() + ".jpg";
                    }
                }
                ...
                else
                {
                    ...
                    using (XmlWriter writer = XmlWriter.Create(FS_Face))
                    {
                        ...
                        writer.WriteElementString("NAME", Newfacename);
        ...
        private void Delete_Data_BTN_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Are you sure, you want to delete all the faces captured from the database?", "Attention!", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if(MessageBox.Show("Are you sure, you want to delete all the faces captured from the database?", "Attention!", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
        ...
        {
            txtLas.Text = "";txtFir.Text = "";txtOth.Text = "";
        }
    }
}
